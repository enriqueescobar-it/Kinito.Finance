...
class AbstractTimeSpanInfo(AbstractTimeInfo):
    ...
    _start_dti: DateTimeInfo = DateTimeInfo(datetime.now())
    _delta_days: int = 0
    ...
    def __init__(self, date_time_start: datetime, date_time_stop: datetime = datetime.now()) -> None:
        ...
        self._start_dti = DateTimeInfo(date_time_start)
        delta_datetime: timedelta = self._start_dti.datetime - self.stop_datetime_info.datetime
    ...
    @property
    def start_datetime_info(self) -> DateTimeInfo:
        return self._start_dti

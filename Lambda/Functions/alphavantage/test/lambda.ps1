#poweshell
python -m venv venv ;
.\venv\Scripts\python.exe -m pip install --upgrade pip ;

.\venv\Scripts\python.exe -m pip install --use-pep517 typing boto3 #pandas pyarrow s3fs prettytable ;

#.\venv\Scripts\pip.exe install pandas -t pandas38/python/lib/python3.8/site-packages ;
#.\venv\Scripts\python.exe -m pip install --use-pep517 -r .\requirements.txt ;

.\venv\Scripts\pip.exe install --use-pep517 typing boto3 pandas pyarrow s3fs prettytable ;
.\venv\Scripts\pip.exe freeze > requirements.txt ;
.\venv\Scripts\pip.exe install --use-pep517 -r .\requirements.txt ;

$Env:AWS_ACCESS_KEY_ID="ASIAUJK4PF4ZBAA4S7W2"
$Env:AWS_SECRET_ACCESS_KEY="CL9oVRIKHimnCeXcOJ5UYKsw4FShifTRhh4bGiXY"
$Env:AWS_SESSION_TOKEN=""

$Env:AWS_REGION="ca-central-1" ;

#Write-Host "aws sts get-caller-identity" ;
#aws sts get-caller-identity ;

Write-Host "aws configure sso" ;
aws configure sso ;

Write-Host "docker build . -t lambda-handler:latest" ;
docker build . -t lambda-handler:latest ;
docker build -f Dockerfile . -t lambda-handler:latest ;

Write-Host "docker run -p 9000:8080 lambda-handler:latest" ;
docker run -p 9000:8080 lambda-handler:latest ;

curl -XPOST "http://localhost:9000/2015-03-31/functions/function/invocations" -d '{"in_bucket": "eescobars3bucket", "in_csv": "jrv-images-id-url.csv"}' ;

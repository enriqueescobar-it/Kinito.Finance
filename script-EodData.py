...
def get_yfinancials(yahoo_symbol, yahoo_file, yahoo_list):
    ...
    if (type(float_peratio) != float):
        ...
        float_num_sh_out = PanYF(
            yahoo_symbol).get_num_shares_outstanding(price_type='current')
        ...
        int_netincfromcontops = PanYF(
            yahoo_symbol).get_net_income_from_continuing_ops()
		...
		f.close()'''
        # DICT
        yahoo_content = PanYF(yahoo_symbol).get_financial_stmts(
            'annual', 'balance', reformat=False)
        yahoo_header = yahoo_head + "Financial Statements (annual,balance) [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(
            yahoo_symbol).get_stock_price_data(reformat=False)
        yahoo_header = yahoo_head + "Stock Price Data [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(
            yahoo_symbol).get_stock_earnings_data(reformat=False)
        yahoo_header = yahoo_head + "Stock Earnings Data [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(yahoo_symbol).get_summary_data(reformat=False)
        yahoo_header = yahoo_head + "Summary Data [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(yahoo_symbol).get_stock_quote_type_data()
        yahoo_header = yahoo_head + "Stock Quote Type Data [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(yahoo_symbol).get_historical_price_data(
            '2019-01-01', '2020-01-01', 'monthly')
        yahoo_header = yahoo_head + "Historical Price Data (last year, monthly) [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(yahoo_symbol).get_daily_dividend_data(
            '2019-01-01', '2020-01-01')
        yahoo_header = yahoo_head + "Daily Dividend Data (last year) [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        yahoo_content = PanYF(yahoo_symbol).get_key_statistics_data()
        yahoo_header = yahoo_head + "Key Statistics Data [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
...
def get_pandr(yahoo_symbol, yahoo_file):
    ...
    yahoo_content = PanDR.get_data_yahoo(yahoo_symbol,
                                         start=PanDateTime.datetime(
                                             2019, 1, 1),
                                         end=PanDateTime.datetime(2020, 1, 1))
    yahoo_header = yahoo_head + "Last Year [" + str(
        type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
    return yahoo_header
...
def get_yfinance(yahoo_symbol, yahoo_desc, yahoo_file):
    ...
    yahoo_header = yahoo_head + "History (min) [" + str(
        type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
    f.write(yahoo_header)
    # SERIES
    yahoo_content = YaFin.Ticker(yahoo_symbol).dividends
    yahoo_header = yahoo_head + "Dividends [" + str(
        type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
    f.write(yahoo_header)
    # SERIES
    ...
    try:
        ...
        yahoo_header = yahoo_head + "Quarterly Cashflow [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        f.write(yahoo_header)
        # DATA_FRAME
        ...
        yahoo_header = yahoo_head + "Quarterly Earnings [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
        f.write(yahoo_header)
        # DATA_FRAME
        ...
        yahoo_header = yahoo_head + "yfinance Info [" + str(
            type(yahoo_content)) + "]\n" + get_the_string(yahoo_content, yahoo_file) + "\n"
    except:
...
tsx_file_ = 'EodData'
tsx_file_ = PanOSPath.join(tsx_file_, 'NYSE')
tsx_file_ = PanOSPath.join(tsx_file_, 'NYSE-y.txt')
tsx_start = PanDateTime.datetime(2019, 1, 1)
tsx_stop_ = PanDateTime.datetime.now()
tsx_today = str(PanDateTime.datetime.now().strftime('%Y%m%d'))
print("#\t" + tsx_file_)

tsx_list_ = list()
tsx_list_ = ["NA"] * 47  # ["NA" for i in range(47)]
...
with open(tsx_file_) as aStockFile:
    # readStockCSV = csv.reader(aStockFile, delimiter='\t')
    readStockCSV = csv.DictReader(aStockFile, delimiter='\t')
    tsx_today = PanOSPath.join('EodData', tsx_today)
    set_directory(tsx_today)
    stockCsvFile = PanOSPath.join(tsx_today, PanOSPath.basename(tsx_file_).replace('.txt', '.csv'))    
    print(stockCsvFile)
    set_file_header(stockCsvFile, headlist)
    for readStockCSVLine in readStockCSV:
        stockSymbol = readStockCSVLine['Symbol']
        stockDescription = readStockCSVLine['Description']
        readStockCSVfile = get_yfile(tsx_file_, stockSymbol)
        tsx_list_[0] = stockSymbol
        tsx_list_[1] = stockDescription
        tsx_list_[2] = get_panda_tgt(stockSymbol)
        readStockCSVfile = PanOSPath.join(PanOSPath.dirname(stockCsvFile), PanOSPath.basename(readStockCSVfile))
        print(readStockCSVfile)
        set_yfile(readStockCSVfile, stockSymbol, stockDescription)
        get_yfinancials(stockSymbol, readStockCSVfile, tsx_list_)
'''
set_yfile(readStockCSVfile, stockSymbol, stockDescription)
get_yfinancials(stockSymbol, readStockCSVfile, tsx_list_)

...
class PortfolioOptim(AbstractPortfolioMeasure):
    _portfolio_limit: int = 5000

    def __init__(self, portfolio_data: DataFrame = DataFrame(), log_ret: DataFrame = DataFrame(),
                 cov_mat: DataFrame = DataFrame()):
        # Creating an empty array to store portfolio weights
        all_wts = np.zeros((self._portfolio_limit, len(portfolio_data.columns)))
        # Creating an empty array to store portfolio returns
        port_returns = np.zeros(self._portfolio_limit)
        # Creating an empty array to store portfolio risks
        port_risk = np.zeros(self._portfolio_limit)
        # Creating an empty array to store portfolio sharpe ratio
        sharpe_ratio = np.zeros(self._portfolio_limit)
        for i in range(self._portfolio_limit):
        ...
        print(port_risk.min())
        min_var = Series(min_var, index=names)
        ...
        plt.show()
        max_sr = Series(max_sr, index=names)

...
class MacdStrategy(AbstractTechIndicatorStrategy):
    ...
    def __init__(self, macd_indicator: MacdIndicator):
        ...
        self._col = self._macd_indicator.GetCol()
        ...
        self._data = a_df[self._macd_indicator.GetCol()].to_frame()
        self._data[self._lower_label] = a_df[self._lower_label]
        #
        self._data[self._upper_label] = a_df[self._upper_label]
        self._buy_label = self._macd_indicator.GetLabel() + self._buy_label
        self._sell_label = self._macd_indicator.GetLabel() + self._sell_label
    ...
    def Plot(self) -> plt:
        ...
        plt.style.use(self._macd_indicator.GetPlotStyle())
        ...
        plt.title(self._macd_indicator.GetMainLabel())
        plt.xlabel(self._macd_indicator.GetXLabel())
        plt.xticks(rotation=self._macd_indicator.GetXticksAngle())
        plt.ylabel(self._macd_indicator.GetYLabel())
        plt.legend(loc=self._macd_indicator.GetLegendPlace())
        return plt

    def PlotAll(self) -> plt:
        n_col: int = 1
        n_row: int = 2
        a_title: str = self._macd_indicator.GetMainLabel()
        x_title: str = self._macd_indicator.GetXLabel()
        y_title: str = self._macd_indicator.GetYLabel()
        ...
        ax[0].legend(loc=self._macd_indicator.GetLegendPlace())
        #ax1 index
        ...
        ax[1].xaxis.set_tick_params(rotation=self._macd_indicator.GetXticksAngle())
        ax[1].set(ylabel='Index', xlabel=x_title)
        ax[1].legend(loc=self._macd_indicator.GetLegendPlace())
        return plt

...
class EmaStrategy(AbstractTechIndicatorStrategy):
    ...
    def __init__(self, ema_indicator: EmaIndicator):
        ...
        self._col = self._ema_indicator.GetCol()
        ...
        self._data = a_df[self._ema_indicator.GetCol()].to_frame()
        ...
        self._buy_label = self._ema_indicator.GetLabel() + self._buy_label
        self._sell_label = self._ema_indicator.GetLabel() + self._sell_label
    ...
    def Plot(self):
        ...
        plt.style.use(self._ema_indicator.GetPlotStyle())
        ...
        plt.title(self._ema_indicator.GetMainLabel())
        plt.xlabel(self._ema_indicator.GetXLabel())
        plt.xticks(rotation=self._ema_indicator.GetXticksAngle())
        plt.ylabel(self._ema_indicator.GetYLabel())
        plt.legend(loc=self._ema_indicator.GetLegendPlace())
        return plt

    def PlotAll(self) -> plt:
        n_col: int = 1
        n_row: int = 2
        a_title: str = self._ema_indicator.GetMainLabel()
        x_title: str = self._ema_indicator.GetXLabel()
        y_title: str = self._ema_indicator.GetYLabel()
        ...
        ax[0].legend(loc=self._ema_indicator.GetLegendPlace())
        #ax1 strategy
        for a_ind, col in enumerate(self._ema_indicator.GetData().columns[-2:self._ema_indicator.GetData().columns.size]):
            an_alpha: float = 0.5 if a_ind != 0 else 1.0
            ax[1].plot(self._ema_indicator.GetData()[col], alpha=an_alpha, label=col)
        ax[1].xaxis.set_tick_params(rotation=self._ema_indicator.GetXticksAngle())
        ax[1].set(ylabel='Index', xlabel=x_title)
        ax[1].legend(loc=self._ema_indicator.GetLegendPlace())
        return plt

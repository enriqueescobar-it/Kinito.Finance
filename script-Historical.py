...
yahooStockOption: YahooStockOption = YahooStockOption('RSG')#'ESTC')XWEB DDOG BEKE GBTC GNDP.V
# DividendYield [2%, 4%]
...
# AAPL ABBV+ AAOI ADBE+ ABR+ ABT++ ABX.TO&GOLD ACES ADC+ ADP AGNC AKAM AMGN AMD+20 AMP AMT+ AMZN++ ANTM+ AQN+ AQN.TO+ AIVSX+
# ARKK+++ ARKF++ ARKG+ ARKQ ARKW++++ APPS+++ APPN+5 AVGO++ AA. APHA+ ASAN ARKF ARKK AVAV AVGO++ AVB AYX++ AZN AWK+ ANET+
# AEM+ AEM.TO+ AT.TO ATZ.TO. ACO-X.TO+ ARKG+ A++ ACO-X.TO. ACN++
# BA BCE BCE.TO. BNS BPY BABA++ BIDU- BBBY BNTX BRK-B BRKA BRKB BRTXQ BYND BNS.TO. BIP+ BZUN+ BILI++ BEP+ BYND+ BAM-A.TO+
# BMO.TO. BTI- BMY- BGS.
# CAG CAT CCA CCA.TO. CAR.UN.TO CGNX CGO CGO.TO. CHD CHWY CL- CLX CM CNI CSU.TO CSX CVD CMCSA COST++ CP CRM CSCO+ CDNS
# CNQ.TO CSIQ CELH+ CM+ CPX.TO++ CVA. CLH+
# CNR.TO+ CU.TO. CSU.TO++ CTC-A.TO+ CLR.TO. CRWD++ CARR++ CB- CU.TO. CRSP++++++ C. CLNE. CMCSA+ CIBR+
# DCBO.TO DND.TO DLHC DDOG DKNG+ DDOG+ DOL.TO+ DND.TO+ DIS- DHR+++ DTE+ DE++ DOCU+3 DVN. DAL. DLR+
# EMR+ ENB- ENB.TO- ESS ESTC+ ESPO EMA.TO ET ETSY+10 ENGH.TO+ ENPH+ EEM EQB.TO+ ELY+ ESS- EPD+ EB. ENPH+25 ET.
# F FD.TO FF.TO FFMG FENY FIE FIVG FROG FSLR FSLY++ FSRNX FSZ FTS+ FTS.TO+ FOOD.TO FZILX FZROX FVRR FXAIX FEYE. FRT. FTEC+2
# GE. GD. GM GFI++ GIS GLD GLW GMF GNW GRWG GILD GBTC GAMR GSY.TO+ GOOD GXC+ GOOG+ GGG++
# HD+ HLT HQU HZU HEO.V HII. HLF.TO. HUBS+++ HRL. HTA. HIW. HACK++
# IBUY IWFH INTC+ IPO IIVI IIPR+++++ INVH+ IVAC IGV+ IBM- IRM+ IT IPFF IWY++ ICLN++ JKS JD+ JNJ+ JPM+ JNUG JNPR
# KEY K KXS.TO KDP KL KL.TO KO. KR KMI- K.TO++ KXS.TO+
# LB.TO LMT LMND LOW+ LSPD++ LSPD.TO++ LVGO LOGI+ LAND+ LMND+ LMT+
# M MA+2 MAIN MELI+ MFI.TO MO- MU MCD- MGM MPW MRK+ MARA MRNA MRVL MSFT+ MWK MDY MRU.TO+ MSCI+ NXST MFC.TO. MXIM+ NWC.TO.
# MDLZ. MDB+6 MAA.
# NEE+ NEM+ NET+2 NKE NSP NEAR NFLX+++ NIO+4 NLY. NLOK NNDM NPI NOBL+ NVEI.TO NVDA+++++ NOW+ NRZ-
# O+ OTEX OHI O- NWC.TO. OKTA+++++ OKE++ MC-
# PM- PANW PFE PINS PGX PEP PLAN PLTR PPL.TO+ PBA+ PRU- PVD PTON PAWZ PSEC PYPL PCTY+++ PKI.TO PIODX+ PKI.TO+ PINS+ PHO PHM++ POW.TO.
# PAAS+++ PAYC+6 PBH. PDD+3 PNC+
# QCOM QQC-f QQQ QRVO QSR.TO+ QQQJ++
# RBA RCI+ RCI.B.TO+ REAL.TO REGI ROK+ RUN RY+ RY.TO+ RCI.B.TO. RIOT RHS QRS.TO REAL.TO ROKU+ RF+ RNW.TO++ ROP+ RSG++
# SU. SU.TO. SIS.TO+ SNA SBUX+ SHOP+20 SHOP.TO+20 SPLK SPYD SJR.B SJR-B.TO SLV+ SNAP SNOW+ SPOT SOXL SRU.UN.TO. SPG SAP
# SPNS STOR+ STAG SEDG+ SJR-B.TO SOY.TO. SYK++ STX+ SQ+8
# T+ T.TO+ TD TDOC+ TEC.TO TAL+ TEAM TFII+ TFII.TO+ TGT- TRMB TSLA+++ TRP TRP.TO TTD+15 TOU.TO TROW TWLO+6 TXN TWST+
# TVE.TO. TPB+++ TSN- TSM+
# U UNM UNP UPWK UNM-
# VDC VFC VFF VFV.TO VGT VMW VNR VPL VPU VTSAX VYM VYMI VZ+ V VOO+ VOOG+ VNQ+ VIOO VHT VSP.TO VEQT.TO+ VGRO.TO+
# W+3 WCN++ WCN.TO++ WELL.TO WEED.TO++ WFG WM++ WMT++ WORK. WPC. XEI XIT XBC.TO XOM. XWEB+ XLK+ XBC.V+++ XTC.TO. ZS++
# YCBD
# ZG ZM+++ ZQQ ZS ZWB ZUO.
